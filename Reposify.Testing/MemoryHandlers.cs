...
namespace Reposify.Testing
{
    public class MemoryHandlers
    {
        ...
        public MemoryHandlers UsingHandlersFromAssemblyForType<T>()
        {
            ...
            foreach (var type in types)
            {
                foreach (var intrface in type.GetInterfaces())
                {
                    ...
                    if (genericType == typeof(IMemoryExecutionHandler<>))
                        _executionHandlers[intrface.GenericTypeArguments[0]] = type;

                    if (genericType == typeof(IMemoryQueryHandler<,>))
        ...
        public virtual void Execute(MemoryRepository repository, IDbExecution dbExecution)
        {
            if (dbExecution == null)
            ...
            if (!_executionHandlers.ContainsKey(type))
                throw new Exception($"no handler found for {type} - ensure there is a handler registered that implements IMemoryExecutionHandler<{type.Name}>");
            ...
            if (execute == null)
                execute = handlerType.GetInterfaces().Where(i => i.Name.StartsWith("IMemoryExecutionHandler")).Single().GetMethod("Execute");

            execute.Invoke(handler, new object[] { repository, dbExecution });
        }

        public virtual TResult Execute<TResult>(MemoryRepository repository, IDbQuery<TResult> dbQuery)
        {
            if (dbQuery == null)
                throw new Exception("attempt to execute null query");
            ...
            if (!_queryHandlers.ContainsKey(type))
                throw new Exception($"no handler found for {type} - ensure there is a handler registered that implements IMemoryQueryHandler<{type.Name}>");
            ...
            if (execute == null)
                execute = handlerType.GetInterfaces().Where(i => i.Name.StartsWith("IMemoryQueryHandler")).Single().GetMethod("Execute");
